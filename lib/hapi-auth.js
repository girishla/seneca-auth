...
module.exports = function (options) {
  ...
  function get_restriction (msg, done) {
    ...
    if (restrict_mode) {
      done(null, {auth: {strategies:internals.strategies}})
    }
    else {
      done()
    }
  ...
  function trigger_service_login (args, respond) {
    ...
    req.seneca.act('role: auth, prepare: ' + service + '_login_data', {
      ...
      delete data.id
      req.seneca.act('role: auth, trigger: service-register-' + service, {user: data}, function (err, data) {
        if (err) {
          return afterlogin(err, respond, req, res)
        }
  ...
  function init (args, done) {
    ...
    internals.seneca.act('role: auth, do: addStrategy',{},function (err, strategies) {

      internals.strategies=strategies
      // seneca web endpoints map
      internals.map = {
        logout: {GET: true, POST: true, data: true, auth: {'strategies': strategies} , alias: options.urlpath.logout},
        register: {POST: true, data: true, alias: options.urlpath.register},
        user: {GET: true, POST: true, auth: {'strategies': strategies}, alias: options.urlpath.user},
        ...
        update_user: {POST: true, data: true, auth: {'strategies': strategies}, alias: options.urlpath.update_user},
        change_password: {POST: true, data: true, auth: {'strategies': strategies}, alias: options.urlpath.change_password}
      }
  ...
  seneca.add('role: auth, cmd: register_service', registerAuthService)

  seneca.add('init: hapi-auth-plugin', init)
